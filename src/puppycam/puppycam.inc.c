...
// Alias old camera name to new one
#define approach_s16_symmetric camera_approach_s16_symmetric
...
u8 newcam_xlu = 255;
s8 newcam_stick2[2];
s16 newcam_rightstick[2]; //Raw right-stick input, filled in by the controller backend.
...
///These are the default settings for Puppycam. They are sourced from the config file on non-N64 targets.
void newcam_init_settings(void)
{
    ...
    newcam_analogue = 1;
    newcam_active = configPuppycam;
}

//Mirror the live settings back into the config variables so they persist on save.
...
//Toggle puppycam on or off at runtime, switching the active camera mode to match.
void newcam_set_active(u8 active)
{
    newcam_active = active;
    if (gCamera != NULL)
    {
        if (active)
        {
...
{
    newcam_stick2[0] = newcam_rightstick[0]*0.625f;
    newcam_stick2[1] = newcam_rightstick[1]*0.625f;
}
